...
- **Расширенный сбор данных 2GIS:**
  ...
  - ID организации и ID записи в 2GIS, мобильный телефон, почтовый индекс, рейтинги/отзывы организации, ссылку на отзывы, координаты (`point_lat`/`point_lon`);
...
- **БД-режим на TimescaleDB/Postgres** (опционально, env `P2GIS_DB_URL`): кэш запросов с
  отдачей из БД без Chrome, планировщик автообновления, поиск из БД, аналитика покрытия,
  синхронизация организаций во внешнюю схему. Подробности — в разделе
  «🗄 БД-режим (TimescaleDB/Postgres)» ниже.
- **Справочники в БД**: в БД-режиме города и рубрикатор (канонически в `p2gis.cities` /
  `p2gis.rubrics`) автообновляются из data.2gis.com; генератор ссылок, планировщик и
  `POST /api/cities` читают/пишут из БД, файлы остаются зеркалом для файлового режима.
  Синк дополнительно связывает филиалы с категориями целевой схемы (`branch_categories`).
...
| POST | `/api/prices` | **Собрать прайс-каталог фирм** с market API 2GIS (без Chrome). Тело: `{"firm_ids": [...], "locale": "ru_RU", "delay"?: N}`. В БД-режиме сохраняет в `p2gis.branch_prices`, в файловом — возвращает без сохранения. Ответ: `{"ok": true, "results": [...]}` |
| POST | `/api/prices/read` | **Прочитать прайс фирмы из БД**. Тело: `{"firm_id": "...", "limit"?: N}`. Ответ: `{"ok", "firm_id", "items", "count"}` |
| POST | `/api/sync` | **Синхронизировать организации в целевую схему** (БД-режим; фильтры `since/limit/city/rubric_id/deactivate/sync_prices`). Подробно — раздел «🗄 БД-режим» |
| GET | `/api/sync/status` | Курсор и последняя ошибка синхронизации: `{enabled, last_synced_at, last_error}` |
...
2GIS отдаёт прайс-каталог филиала отдельным API — `market-backend.api.2gis.ru`
(`items_by_branch`), без Chrome и без авторизации:

```bash
# Собрать прайс-каталог фирмы (в БД-режиме сохраняется в p2gis.branch_prices)
curl -X POST http://127.0.0.1:8666/api/prices -H 'Content-Type: application/json' -d '{
  "firm_ids": ["9148465024074680"],
  "locale": "ru_RU"
}'

# Прочитать прайс из БД
curl -X POST http://127.0.0.1:8666/api/prices/read -H 'Content-Type: application/json' -d '{
  "firm_id": "9148465024074680", "limit": 100
}'
```

- `POST /api/prices` собирает товары/услуги с ценами, категориями и изображениями.
  В БД-режиме результат сохраняется в `p2gis.branch_prices` (upsert по
  `firm_id`+`product_id`), в файловом режиме — возвращается без сохранения.
- `POST /api/prices/read` читает сохранённые позиции из БД.
- Собранный прайс можно перенести в целевую схему через `POST /api/sync`
  (с `sync_prices: true`) — см. раздел «🗄 БД-режим».
...
---
## 🗄 БД-режим (TimescaleDB/Postgres)

Опциональное хранилище результатов на **TimescaleDB 17** (обычный PostgreSQL также подходит),
дающее функциональность, которой нет в файловом режиме. Файловый режим остаётся **по умолчанию**
и полностью сохранён.

### Включение

```bash
export P2GIS_DB_URL='postgres://user:password@host:5432/db'
parser-2gis-new            # веб-интерфейс: БД-режим включён автоматически
parser-2gis-new --storage db -i <URL> -o out.csv -f csv   # CLI в БД
```

Схема `p2gis` (таблицы `records`, `request_cache`, `parse_requests` — гипертаблица TimescaleDB,
`refresh_schedules`, `sync_state`, `cities`, `rubrics`, `branch_prices`) применяется
идемпотентно при старте,
а справочники **сразу сидируются в БД из файлов** (`p2gis.cities` / `p2gis.rubrics`), после чего
обновляются из data.2gis.com. В вебе режим можно выбрать на задачу:
**Расширенные настройки → Хранилище**, либо `advanced.storage: 'db'` в `POST /api/start`;
по умолчанию («Авто») при заданном `P2GIS_DB_URL` используется `db`.

### Что даёт

- **Кэш запросов.** Повторный запуск того же запроса (город + рубрика `rubricId` или
  нормализованные ключевые слова) в пределах TTL (по умолчанию 7 дней,
  `P2GIS_CACHE_TTL_HOURS` / `advanced.cache_ttl_hours`) отдаёт результат **из БД без запуска
  Chrome** — ответ `POST /api/start` содержит `cache_hit: true`. Дедуп между городами/задачами
  встроен в схему: `records.firm_id` — первичный ключ, upsert.
  Запросы **без городского префикса** (`https://2gis.ru/search/...`, в т.ч. координатные
  `?m=lon,lat/zoom`) не кэшируются — они охватывают регион/точку и всегда парсятся заново.
- **Планировщик автообновления.** Вкладка «🗄 БД» в вебе (или REST `/api/schedules`): cron
  (`0 3 * * *`) или интервал в минутах, города × рубрики/запросы, лимит воркеров, свой TTL,
  `sync_after`. Фоновый цикл запускает due-задачи через общий JobManager.
- **Синхронизация во внешнюю целевую схему.** `POST /api/sync` переносит организации целиком
  (org + филиалы) в схему назначения: upsert по `firm_id`, деактивация филиалов сети,
  отсутствующих в свежем наборе, курсор `p2gis.sync_state`. Имя схемы задаётся через
  `P2GIS_SYNC_SCHEMA` (обязательно; без неё синхронизация не запускается).
  Дополнительно связывает филиалы с категориями схемы по коду рубрики
  (`branch_categories`, в ответе `categories_linked`).
  Автосинк после каждой задачи — `advanced.sync_after` (по умолчанию вкл в БД-режиме).
- **Справочники в БД.** В БД-режиме города и рубрикатор канонически хранятся в
  `p2gis.cities` / `p2gis.rubrics`: автообновление (при старте, раз в сутки, `POST /api/refresh`)
  пишет и в БД, и в файлы (зеркало для файлового режима/офлайн). Генератор ссылок, `/api/cities`
  и планировщик читают из БД. Добавленный через `POST /api/cities` город попадает в БД
  (`source='custom'`) и в `cities_custom.json`. Регион города (`region`) заполняется
  автоматически из спарсенных записей (город → регион административного деления) и доступен
  в `/api/cities` и конструкторе ссылок.
- **Прайс-каталог в БД.** Собранный через `POST /api/prices` прайс сохраняется в
  `p2gis.branch_prices` (upsert, поиск по названию через pg_trgm, фильтр по категориям,
  вкладка «Цены» в вебе). `POST /api/sync` (с `sync_prices: true`) переносит его в целевую
  схему и сводит min/max цены филиалов.
- **Поиск из БД** `GET /api/db/search?city=&q=&rubric=` — pg_trgm по названию/адресу/рубрикам,
  без Chrome.
- **Аналитика:** `GET /api/db/cache` (свежесть запросов), `GET /api/db/coverage`
  (город × рубрика: записей, последнее обновление), `POST /api/refresh-stale` (пере-парсинг
  всех протухших запросов).
- **seen_firms на БД:** `--parser.skip-seen-firms` в БД-режиме читает свежие (< TTL) фирмы
  из `p2gis.records` вместо файлового кэша.

### Экспорт файловой истории

```bash
P2GIS_DB_URL='postgres://...' python scripts/backfill_history_to_db.py
```
Импортирует `~/.local/share/parser-2gis/history/*.json` в `p2gis.records` и заполняет кэш запросов.

> Ключевые слова-«резервы»: `query` — зарезервировано в Litestar, поэтому поиск из БД использует
> параметр `q`.
