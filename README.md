...
### 🧵 Параллельные запросы и прокси

- **Конкурентный парсинг:** несколько задач одновременно (лимит `max_concurrent`), задачи встают в очередь, у каждой свой Chrome и свой `job_id`.
- **Прокси:** список прокси в панели (сохраняется в localStorage) или в API (`proxies`). Каждой параллельной задаче назначается один прокси по round-robin и Chrome стартует с `--proxy-server=...`.
- **Важно:** прокси работают **только для параллельных запросов** — каждый job использует свой прокси на протяжении всей сессии. Формат: `user:pass@host:port` (HTTP) или `host:port` (SOCKS5 через `socks5://host:port`).
