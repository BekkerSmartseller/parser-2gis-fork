...
- **БД-режим на TimescaleDB/Postgres** (опционально, env `P2GIS_DB_URL`): кэш запросов с
  ...
  синхронизация `p2gis → medexpertai`. Подробности — в разделе
...
- **Справочники в БД**: в БД-режиме города и рубрикатор (канонически в `p2gis.cities` /
  ...
  Синк дополнительно связывает филиалы с категориями health_ai (`branch_categories`).
...
`refresh_schedules`, `sync_state`, `cities`, `rubrics`) применяется идемпотентно при старте,
...
- **Синхронизация `p2gis → medexpertai`.** `POST /api/sync` переносит организации целиком
  (org + филиалы) в схему `medexpertai` (единая база организаций health_ai): upsert по `firm_id`,
  деактивация филиалов сети, отсутствующих в свежем наборе, курсор `p2gis.sync_state`.
  Дополнительно связывает филиалы с `medexpertai.categories` по коду рубрики
...
- **Справочники в БД.** В БД-режиме города и рубрикатор канонически хранятся в
  ...
  (`source='custom'`) и в `cities_custom.json`.
