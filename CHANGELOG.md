...
- Ротация прокси между задачами (round-robin). Chrome запускается с `--proxy-server=...`. Прокси передаются в `/api/start` (`proxies`) и сохраняются в UI (localStorage).
- ВАЖНО: прокси работают только для параллельных запросов — каждая задача получает один прокси на весь парсинг.
